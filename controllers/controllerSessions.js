...
const createSession = async (req, res) => {
  const { hall_id, hall_title, session_date, session_start, session_finish } = req.body;
  try {
    if (session_start >= session_finish) {
      console.log(`Session start ${session_start} is not before finish ${session_finish}`);
      return res.status(400).json({ error: `Session start ${session_start} must be before finish ${session_finish}` });
    }
...
const updateSession = async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount > 0) {
      ...
      return res.status(200).json({ message: `Session with ID ${id} updated successfully.` });
    } else {
...
const deleteSession = async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount > 0) {
      ...
      return res.status(200).json({ message: `Session with ID ${id} deleted successfully` });
    } else {
      console.log(`Session with ID ${id} does not exist`);
      return res.status(404).json({ error: `Session with ID ${id} does not exist` });
    }
  } catch (err) {
